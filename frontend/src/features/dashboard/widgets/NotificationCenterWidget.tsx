...
export function NotificationCenterWidget() {
  const { unreadAlertsCount, setUnreadAlertsCount } = useDashboardStore();
      ...
      <CardContent className="flex-1 overflow-y-auto pt-2 space-y-3">
        {unreadAlertsCount > 0 ? (
          <div className="space-y-3">
            <div className="p-3 border rounded bg-red-950/10 border-red-500/15 flex gap-2 text-xs">
              <ShieldAlert className="w-4.5 h-4.5 text-red-400 shrink-0" />
              <div className="space-y-1">
                <p className="font-semibold text-slate-200">Critical: Prod Database Cluster Peak Memory</p>
                <p className="text-[10px] text-muted-foreground">Threshold 95% exceeded on auth-db-primary</p>
              </div>
            </div>
          </div>
        ) : (
