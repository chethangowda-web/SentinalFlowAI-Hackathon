...
import { Cpu, BookOpen, TrendingUp, BarChart3, Lightbulb, AlertTriangle, CheckCircle } from 'lucide-react';
import { Card, CardContent, CardHeader, CardTitle } from '@/components/ui/card';
import { Skeleton } from '@/components/ui/skeleton';
import { useLearning } from '@/hooks/useLearning';
import { BarChart, Bar, ResponsiveContainer, XAxis, YAxis, Tooltip as RechartsTooltip, AreaChart, Area } from 'recharts';
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <Card className="bg-card border-border/50">
          <CardContent className="p-4 flex items-center gap-3">
            ...
            <div>
              <p className="text-xs text-muted-foreground font-medium">Incidents Learned</p>
              <p className="text-xl font-bold font-mono">{overviewData.totalIncidentsLearned}</p>
            </div>
          </CardContent>
        </Card>
        <Card className="bg-card border-border/50">
          <CardContent className="p-4 flex items-center gap-3">
            ...
            <div>
              <p className="text-xs text-muted-foreground font-medium">Embeddings</p>
              <p className="text-xl font-bold font-mono">{overviewData.embeddingsGenerated.toLocaleString()}</p>
            </div>
          </CardContent>
        </Card>
        <Card className="bg-card border-border/50">
          <CardContent className="p-4 flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xl font-bold font-mono">{overviewData.knowledgeBaseSize} docs</p>
            </div>
          </CardContent>
        </Card>
        <Card className="bg-card border-border/50">
          <CardContent className="p-4 flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xl font-bold font-mono">{overviewData.similarityAccuracy}%</p>
            </div>
          </CardContent>
        </Card>
      </div>
            ...
            <div className="h-64">
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={growthData}>
                  ...
                  />
                  <Area type="monotone" dataKey="embeddings" stroke="#2563eb" strokeWidth={2} fill="url(#embeddingGradient)" name="Embeddings" />
                </AreaChart>
              </ResponsiveContainer>
            </div>
          </CardContent>
              ...
              {overviewData.topRootCauses.map((item, i) => (
                <div key={item.cause} className="flex items-center gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-xs truncate">{item.cause}</p>
                    <div className="w-full h-1.5 rounded-full bg-muted mt-1 overflow-hidden">
                      <div
                        className="h-full rounded-full bg-primary"
                        style={{ width: `${(item.count / Math.max(...overviewData.topRootCauses.map(c => c.count))) * 100}%` }}
                      />
                    </div>
                  </div>
        ...
        <Card className="bg-card border-border/50">
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-semibold flex items-center gap-2">
              ...
              Previously Successful Runbooks
            </CardTitle>
          </CardHeader>
          <CardContent>
            <div className="h-48">
